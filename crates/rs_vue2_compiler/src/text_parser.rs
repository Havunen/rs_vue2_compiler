...
lazy_static! {
    static ref REGEX_CACHE: Mutex<HashMap<(String, String), Arc<Regex>>> = Mutex::new(HashMap::new());
...
fn build_regex(delimiters: (String, String)) -> Arc<Regex> {
    ...
    let regex = Arc::new(Regex::new(&format!(
        "{}((?:.|\\n)+?){}",
        regex::escape(&open),
        regex::escape(&close)
    ))
...
pub fn parse_text(text: &str, delimiters: Option<(String, String)>) -> Option<(String, Vec<String>)> {
    ...
    for cap in tag_re.captures_iter(text) {
        ...
        let exp = parse_filters(&(cap[1].trim().to_string().into_boxed_str(), QuoteType::NoValue));
